...
$usnDump | Where-Object { ($_.UpdateReasons -match 'Rename(OldName|NewName)') -and ($_.Extension -eq '.exe') } | Sort-Object UpdateTimestamp | ForEach-Object { "$([IO.Path]::GetFileName($_.FilePath))`t$($_.UpdateTimestamp.ToString('dd.MM.yyyy HH:mm:ss'))`t$((if ($_.UpdateReasons -match 'RenameOldName') {'Rename: old name'} else {'Rename: new name'}))" } | Out-File RenamedExe.txt -Width 4096 -Append
$usnDump | Where-Object { $_.'FilePath' -match 'System32' -and ($_.Extension -eq '.exe' -or $_.Extension -eq '.dll') } | Sort-Object 'UpdateTimestamp' -Descending -Unique | Out-File System32.txt -Width 4096
$usnDump | Where-Object { $_.'FilePath' -match 'SysWOW64' -and ($_.Extension -eq '.exe' -or $_.Extension -eq '.dll') } | Sort-Object 'UpdateTimestamp' -Descending -Unique | Out-File SysWOW64.txt -Width 4096
